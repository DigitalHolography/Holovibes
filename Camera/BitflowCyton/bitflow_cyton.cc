...
static void print_BiError(Bd board, BFRC status)
{
    ...
    std::cout << "ERROR " << std::string(error_text, error_text_size) << std::endl;
}
    ...
    : Camera("bitflow.ini")
{
    ...
    {
        std::cout << "ERROR Could not open bitflow.ini config file" << std::endl;
...
void CameraPhantomBitflow::create_buffers()
{
    ...
    if (frames == NULL)
    {
        std::cout << "ERROR Could not allocate pointers buffer" << std::endl;
        throw CameraException(CameraException::NOT_INITIALIZED);
    }
    ...
    {
        std::cout << "ERROR Could not allocate data buffer" << std::endl;
        free(frames);
        throw CameraException(CameraException::NOT_INITIALIZED);
    }
...
void CameraPhantomBitflow::load_ini_params()
{
    ...
    {
        std::cout << "ERROR bitflow.ini: number_of_boards should be 1, 2 or 4, you may want to setup the bitflow.ini "
                     "config file"
                  << std::endl;
        throw CameraException(CameraException::NOT_INITIALIZED);
    }
    ...
    for (size_t i = 0; i < nb_boards; ++i)
    {
        ...
        if (board_nums[i] == -1)
        {
            std::cout << "ERROR bitflow.ini: board" << i << "has an invalid value";
            throw CameraException(CameraException::NOT_INITIALIZED);
        }
    }
}
...
void CameraPhantomBitflow::bind_params()
{
    ...
    if (rc != BI_OK)
    {
        std::cout << "Could not read frame description" << std::endl;
        throw CameraException(CameraException::NOT_INITIALIZED);
    }
