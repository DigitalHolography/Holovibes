...
#define IS_NETWORK_FRAME_SIZE_INCOMPATIBLE                                                                             \
...
#define IS_SUBSAMPLING_MASK_VERTICAL                                                                                   \
...
#define IS_SUBSAMPLING_MASK_HORIZONTAL                                                                                 \
...
#define IS_BINNING_MASK_VERTICAL                                                                                       \
...
#define IS_BINNING_MASK_HORIZONTAL                                                                                     \
...
#define DR_ CHECK_COMPATIBILITY 16
...
#define DR_ CHECK_COLOR_MODE_SUPPORT 27
...
// on the status flags
#define IS_CAMERA_AVAILABLE(_s_)                                                                                       \
    ((((_s_)&FIRMWARE_DOWNLOAD_NOT_SUPPORTED) == 0) && (((_s_)&INTERFACE_SPEED_NOT_SUPPORTED) == 0) &&                 \
     (((_s_)&INVALID_SENSOR_DETECTED) == 0) && (((_s_)&AUTHORIZATION_FAILED) == 0))
...
// ----------------------------------------------------------------------------
    ...
    IDSEXP is_GetColorConverter(
        HIDS hCam, INT ColorMode, INT* pCurrentConvertMode, INT* pDefaultConvertMode, INT* pSupportedConvertModes);
    ...
    typedef enum E_FOCUS_CMD
    {
        ...
        FOC_CMD_GET_AUTOFOCUS_ZONE_POS_INC =
            22, /* Get the incrementation for the positions of the measurement window   */
     ...
     * \param   nCommand        code, indicates requested access and accessed value, one from \ref TRANSFER_CMD
     * enumeration. \param   pParam          input or output storage for the accessed param. \param   cbSizeOfParam size
     * of *pParam. \return  error code
     *
     * \note For 'set' commands: If the resulting transfer engine setup differs from the current state the transfer
     * param is immediately written to the device. \note For 'set' commands: The function forces the value to the next
     * lower catch according to the related increment.
     */
...
#define IS_BOOTBOOST_IDLIST_ELEMENTSIZE (sizeof(IS_BOOTBOOST_ID))
     ...
     * \param   nCommand        code, indicates requested access and accessed parameter, one of \ref BOOTBOOST_CMD
     * enumeration. \param   pParam          input or output storage for the accessed param. \param   cbSizeOfParam size
     * of *pParam. \return  error code
     *
     * \since uEye SDK 3.90.
...
#pragma pack(pop)
     ...
     *
     * For data processing callbacks a pointer to this data is provided to the %Callback function as the second
     * argument.
     *
     * \sa IS_CALLBACK_FUNC
     *
     ...
     *
     * For image processing callbacks a pointer to this data is provided to the %Callback function as the second
     * argument.
     *
     ...
     *
     * For image processing callbacks a pointer to this data is provided to the %Callback function as the third
     * argument. The callback function is required to fill provide this information to the API.
     *
     * This type is used for the following callback events:
...
#pragma pack(push, 1)
    ...
    typedef enum _UEYE_ETH_CONTROLSTATUS
    {
        ...
        IS_ETH_CTRLSTATUS_ACCESSIBLE2 = 0x00000004, /*!< device is accessible BY SELF, i.e. not on persistent ip and
                                                       adapters ip autocfg range is valid */
...
#pragma pack(pop)
    ...
    typedef enum E_CONFIGURATION_CMD
    {
        ...
        IS_CONFIG_CPU_IDLE_STATES_CMD_SET_DISABLE_ON_OPEN =
            4, /*!< Disable migration to other CPU idle states (other than C0) if the first USB camera is being opened*/
...
#define IS_LUT_STATE_ID_HARDWARE_LUTANDGAMMA                                                                           \
    (IS_LUT_STATE_ID_FLAG_HARDWARE | IS_LUT_STATE_ID_FLAG_LUT | IS_LUT_STATE_ID_FLAG_GAMMA)
...
#define IS_LUT_STATE_ID_SOFTWARE_LUTANDGAMMA                                                                           \
    (IS_LUT_STATE_ID_FLAG_SOFTWARE | IS_LUT_STATE_ID_FLAG_LUT | IS_LUT_STATE_ID_FLAG_GAMMA)
