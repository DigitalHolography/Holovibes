...
footer_exclude_path = [
    "root['compute_settings']['image_rendering']['space_transformation']",
    "root['compute_settings']['view']['registration']",
    "root['info']['camera_fps']",
    "root['compute_settings']['view']['window']['xy']['contrast']['min']",
    "root['compute_settings']['view']['window']['xy']['contrast']['max']",
]
...
class HoloFile:
    ...
    @staticmethod
    def __get_numpy_array(
        frame: bytes, bits_per_pixel: int, w: int, h: int
    ) -> np.dtype:
        ...
        return np.frombuffer(frame, dtype=meta_data[bits_per_pixel][0]).reshape(
            meta_data[bits_per_pixel][1]
        )
    ...
    def to_file(self, path: str) -> None:
        ...
        io.write(pack(str(holo_header_padding_size) + "s", b"0"))  # padding
        ...
        if "values_changed" in ddiff:
            ...
            for path in [contrast_max_path, contrast_min_path]:
                if path in ddiff["values_changed"]:
                    ...
                    if (
                        abs(diff["new_value"] / diff["old_value"] - 1)
                        <= constant_name.CONTRAST_MAX_PERCENT_DIFF
                    ):
    ...
    def assertHolo(ref, chal: "HoloFile", basepath: str):
        ...
        for attr in ("width", "height", "bytes_per_pixel", "nb_images"):
            if getattr(ref, attr) != getattr(chal, attr):
                errors.append(
                    f"{attr} differ: {getattr(ref, attr)} != {getattr(chal, attr)}"
                )
        ...
        for i, (l_image, r_image) in enumerate(zip(ref.images, chal.images)):
            ...
            if diff > 10**-6:
                ...
                Image.fromarray(l_image).save(
                    os.path.join(basepath, constant_name.REF_FAILED_IMAGE)
                )
                Image.fromarray(r_image).save(
                    os.path.join(basepath, constant_name.OUTPUT_FAILED_IMAGE)
                )
                Image.fromarray(diffMatrix).save(
                    os.path.join(basepath, constant_name.DIFF_FAILED_IMAGE)
                )
                break
...
class HoloLazyReader(HoloLazyIO):
    ...
    def get_all_bytes(self) -> Tuple[bytes, bytes, bytes]:
        data_total_size = (
            self.nb_images * self.height * self.width * self.bytes_per_pixel
        )
        self.io.seek(0)
