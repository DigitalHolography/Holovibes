...
namespace gui
{
	...
	void MainWindow::set_flowgraphy_mode()
	{
		if (is_enabled_camera_)
		{
			holovibes_.get_compute_desc().compute_mode = holovibes::ComputeDescriptor::compute_mode::FLOWGRAPHY;
			QPoint pos(0, 0);
			unsigned int width = 512;
			unsigned int height = 512;
			init_image_mode(pos, width, height);
	...
	void MainWindow::set_image_mode()
	{
		...
		if (holovibes_.get_compute_desc().compute_mode == holovibes::ComputeDescriptor::compute_mode::FLOWGRAPHY)
			set_demodulation_mode();
	}
