...
__device__
void warp_primitive_reduce(T* sdata, const uint tid)
{
...
__global__
void kernel_reduce(const T* const __restrict__ input,
...
void gpu_reduce(const float* const input, double* const result, const uint size)
{
	...
	assert(optimal_block_size >= 64 && "kernel reduce only works with with a block size equal or greater than 64 threads (128 pixels)");
	...
	kernel_reduce<float, double, optimal_block_size * 2><<<nb_blocks, optimal_block_size, optimal_block_size * sizeof(float)>>>(input, result, size);
	cudaCheckError();
}