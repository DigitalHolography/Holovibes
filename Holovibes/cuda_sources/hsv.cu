...
void apply_gaussian_blur(
    float* gpu_arr, uint height, uint width, const holovibes::CompositeHSVStruct& hsv_struct, const cudaStream_t stream)
{
    ...
    float* blur_matrix;
    const float kernel_size = hsv_struct.h.blur.enabled;
