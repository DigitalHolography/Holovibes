                        ...
                        const bool normalize_enabled)
{
    ...
    cudaSafeCall(cudaMemcpy2D(
        cuComplex_buffer,  // Destination memory address
    ...
    cufftSafeCall(cufftExecC2C(plan->get(),
                               cuComplex_buffer,
                               cuComplex_buffer,
                               CUFFT_FORWARD));
    // At this point, cuComplex_buffer is the FFT of the input

    kernel_multiply_frames_complex<<<blocks, threads>>>(cuComplex_buffer,
                                                        gpu_kernel,
                                                        cuComplex_buffer,
                                                        size);
    cudaCheckError();
    ...
    {
        kernel_complex_to_modulus<<<blocks, threads>>>(cuComplex_buffer,
                                                       gpu_convolved_buffer,
                                                       size);
        cudaCheckError();
        kernel_divide_frames_float<<<blocks, threads>>>(gpu_input,
                                                        gpu_convolved_buffer,
                                                        gpu_input,
                                                        size);
    }
