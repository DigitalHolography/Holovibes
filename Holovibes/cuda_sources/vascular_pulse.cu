...
void multiply_three_vectors(float* output, float* input1, float* input2, float* input3, size_t size, cudaStream_t stream)
{
    uint threads = get_max_threads_1d();
    uint blocks = map_blocks_to_problem(size, threads);
...
void compute_first_correlation(float* output, float* image_centered, float* vascular_pulse, int nnz_mask_vesslness_clean, size_t length_video, size_t image_size, cudaStream_t stream)    // Size here is future time window
{
    ...
    cudaXMemcpyAsync(vascular_pulse_copy, vascular_pulse, sizeof(float) * length_video, cudaMemcpyDeviceToDevice, stream);
