...
* Our code does extra useless calculus but introducing branching inside a warp calulation
* Would be slower than making some useless calculations like we are doing
*
...
*/
static __device__
void reduce_full_width_tile(volatile float tile[32][32], const ushort x_tile, const ushort y_tile)
{
 ...
 */
static __device__
void reduce_width_tile(float tile[32][32], const ushort x_tile, const ushort y_tile, const ushort tile_width)
{
	...
	while (stride > 0)
	{
		...
		// We need to ceil for the calculations, but ceil(1/2) = 1 resulting in an infinite loop
		if (stride == 1)
			return;
...
*
* Each thread warp handles a line of the tile and executes a reduce to have the sum
* at index 0 of the line
*
* Each first thread of the warp then writes its line cumulative sum inside the first cell of the tile ([0][0])
*
* Then each tile writes it's cumulative sum in the total_sum output
*/
template <typename FUNC>
static __global__
void kernel_apply_mapped_zone_sum(const float	*input,
	const uint		width,
	double			*total_sum,
	...
	FUNC 			element_map)
{
	...
	if (x_zone < zone_width && y_zone < zone_height)
	{
		...
		// No __syncthreads() is needed since we work at warp level, it's sync by hardware constraint
		...
		__syncthreads();
		/* x_tile == 0 because only the first thread of each line should write its accumulated value
		   y_tile != 0 because the first line already has its accumalted value in tile[0][0] */
		...
		// Only first thread of each tile accumulate the tile result in the output
		if (x_tile == 0 && y_tile == 0)
			  ...
			  const cudaStream_t stream)
{
	...
	const dim3 grid_size(static_cast<ushort>(ceil(static_cast<float>(zone.width()) / static_cast<float>(block_width))),
							...
							const cudaStream_t stream)
{
	...
	double cpu_avg_zone;
	cudaXMemcpy(&cpu_avg_zone, gpu_sum_zone, sizeof(double), cudaMemcpyDeviceToHost);
						...
						const cudaStream_t stream)
{
	...
	double cpu_std_zone;
	cudaXMemcpy(&cpu_std_zone, gpu_std_sum_zone, sizeof(double), cudaMemcpyDeviceToHost);
