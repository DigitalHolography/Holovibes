                                          ...
                                          const size_t input_size)
{
    ...
    {
        output[index] =
            (input[index_10bit] << 8) | ((input[index_10bit + 1] & 0xC0));
        output[index + 1] = ((input[index_10bit + 1] & 0x3F) << 10) |
                            ((input[index_10bit + 2] & 0xF0) << 2);
        output[index + 2] = ((input[index_10bit + 2] & 0x0F) << 12) |
                            ((input[index_10bit + 3] & 0xFC) << 4);
        output[index + 3] = ((input[index_10bit + 3] & 0x03) << 14) |
                            ((input[index_10bit + 4] << 6));
    }
}
