...
namespace holovibes::worker
{
    ...
    void FileFrameReadWorker::run()
    {
        ...
        std::string input_descriptor_info = std::to_string(fd.width) + std::string("x")
        ...
        info.add_progress_index(InformationContainer::ProgressType::FILE_READ, std::ref(current_nb_frames_read_), std::ref(total_nb_frames_to_read_));
    ...
    size_t FileFrameReadWorker::read_copy_file(size_t frames_to_read)
    {
        ...
        try
        {
            frames_read = input_file_->read_frames(cpu_frame_buffer_, frames_to_read);
        ...
        {
            LOG_WARN("[READER] " + std::string(e.what()));
        }
    ...
    void FileFrameReadWorker::enqueue_loop(size_t nb_frames_to_enqueue)
    {
        ...
        while (frames_enqueued < nb_frames_to_enqueue && !stop_requested_)
        {
            ...
            if (!gpu_input_queue_.load()->enqueue(gpu_frame_buffer_ + frames_enqueued * frame_size_,
