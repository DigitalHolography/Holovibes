...
namespace holovibes
{
		...
		if (is_no_gui)
		{
			...
			if (!is_import_mode_enable)
			{
				features_opts_desc_.add_options()
					...
					po::value<std::string>()
					->required(),
					"Set the camera to use: xiq/ids/adimec.")
					;
			}
		}
		else
	...
	void OptionsParser::proceed_compute()
	{
		...
		{
			const unsigned short time_transformation_size = vm_["time_transformation_size"].as<unsigned short>();
			if (time_transformation_size <= 0)
				throw std::runtime_error("--time_transformation_size parameter must be strictly positive");

			opts_.cd.time_transformation_size = time_transformation_size;

			if (opts_.cd.time_transformation_size >= global::global_config.input_queue_max_size)
				throw std::runtime_error("--time_transformation_size can not be greater than the input_max_queue_size");
		}

		if (vm_.count("pindex"))
		{
			const unsigned short pindex = vm_["pindex"].as<unsigned short>();
			if (pindex < 0 || pindex >= opts_.cd.time_transformation_size)
				throw std::runtime_error("--pindex parameter must be defined in {0, ..., time_transformation_size - 1}.");
	...
	{
		if (!vm_.count("time_transformation_size"))
			throw std::runtime_error("--time_transformation_size is required");
