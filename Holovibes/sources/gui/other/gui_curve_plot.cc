    ...
    , auto_scale_curr_points_(0)
{
    ...
    chart->axes(Qt::Horizontal).at(0)->setRange(0, points_nb_);
    chart->axes(Qt::Horizontal).at(0)->setReverse(true);
...
void CurvePlot::resize_plot(const int size)
{
    ...
    // FIXME we take first vertical axis, verify
    chart->axes(Qt::Vertical).at(0)->setMax(QVariant(points_nb_));
}
...
void CurvePlot::auto_scale()
{
    ...
    chart->axes(Qt::Vertical).at(0)->setRange(min - offset, max + offset);
}
