...
void ViewPanel::on_notify()
{
    ...
    update_img_type(static_cast<int>(api.compute.get_img_type()));

    ui_->PhaseUnwrap2DCheckBox->setVisible(api.compute.get_img_type() == ImgType::PhaseIncrease ||
                                           api.compute.get_img_type() == ImgType::Argument);

    ui_->TimeTransformationCutsCheckBox->setChecked(!is_raw && api.view.get_cuts_view_enabled());
    ...
    ui_->FFTShiftCheckBox->setChecked(api.transform.get_fft_shift_enabled());
    ui_->FFTShiftCheckBox->setEnabled(true);

    ui_->RegistrationCheckBox->setChecked(api.global_pp.get_registration_enabled());
    ui_->RegistrationCheckBox->setEnabled(true);
    ui_->RegistrationZoneSpinBox->setEnabled(api.global_pp.get_registration_enabled());
    ui_->RegistrationZoneSpinBox->setValue(api.global_pp.get_registration_zone());

    ui_->LensViewCheckBox->setChecked(api.view.get_lens_view_enabled());
    ui_->LensViewCheckBox->setEnabled(is_data_not_moments);

    ui_->RawDisplayingCheckBox->setChecked(!is_raw && api.view.get_raw_view_enabled());
    ui_->RawDisplayingCheckBox->setEnabled(!is_raw && is_data_not_moments);

    // Contrast
    ui_->ContrastCheckBox->setChecked(!is_raw && api.contrast.get_contrast_enabled());
    ui_->ContrastCheckBox->setEnabled(true);
    ui_->AutoRefreshContrastCheckBox->setChecked(api.contrast.get_contrast_auto_refresh());
    ui_->InvertContrastCheckBox->setChecked(api.contrast.get_contrast_invert());
    ...
    ui_->LogScaleCheckBox->setChecked(!is_raw && api.contrast.get_log_enabled());
    ...
    if (api.view.get_current_window_type() == WindowKind::Filter2D)
        set_xyzf_visibility(false);
    else
    {
        set_xyzf_visibility(true);

        ui_->ImgAccuSpinBox->setValue(api.window_pp.get_accumulation_level());

        constexpr int max_digit_rotate = 3;
        constexpr int max_digit_flip = 1;

        std::string rotation_degree = std::to_string(static_cast<int>(api.window_pp.get_rotation()));
        ...
        current_flip.replace(current_flip.size() - 1,
                             max_digit_flip,
                             std::to_string(api.window_pp.get_horizontal_flip()).c_str());
    ...
    // p accu
    ui_->PAccSpinBox->setValue(api.transform.get_p_accu_level());
    ui_->PSpinBox->setValue(api.transform.get_p_index());
    ui_->PAccSpinBox->setEnabled(api.compute.get_img_type() != ImgType::PhaseIncrease &&
    ...
    api.transform.check_p_limits(); // FIXME: May be moved in setters

    // Enforce maximum value for p_index and p_accu_level
    ui_->PSpinBox->setMaximum(api.transform.get_time_transformation_size() - api.transform.get_p_accu_level() - 1);
    ui_->PAccSpinBox->setMaximum(api.transform.get_time_transformation_size() - api.transform.get_p_index() - 1);
    ui_->PSpinBox->setEnabled(!is_raw && api.compute.get_img_type() != ImgType::Composite);
    ui_->PSpinBox->setVisible(is_data_not_moments);
    ui_->PLabel->setVisible(is_data_not_moments);

    // q accu
    bool is_q_visible =
        api.transform.get_time_transformation() == TimeTransformation::SSA_STFT && !is_raw && is_data_not_moments;
    ...
    ui_->Q_AccSpinBox->setValue(api.transform.get_q_accu_level());
    ui_->Q_SpinBox->setValue(api.transform.get_q_index());

    api.transform.check_q_limits(); // FIXME: May be moved in setters

    // Enforce maximum value for p_index and p_accu_level
    ui_->Q_SpinBox->setMaximum(api.transform.get_time_transformation_size() - api.transform.get_q_accu_level() - 1);
    ui_->Q_AccSpinBox->setMaximum(api.transform.get_time_transformation_size() - api.transform.get_q_index() - 1);

    // XY accu
    ui_->XAccSpinBox->setValue(api.transform.get_x_accu_level());
    ui_->YAccSpinBox->setValue(api.transform.get_y_accu_level());

    int max_width = 0;
    int max_height = 0;
    if (api.compute.get_input_queue() != nullptr)
    {
        max_width = api.input.get_fd().width - 1;
        max_height = api.input.get_fd().height - 1;
    }
    else
        api.transform.set_x_y(0, 0);

    ui_->XSpinBox->setMaximum(max_width);
    ui_->YSpinBox->setMaximum(max_height);
    QSpinBoxQuietSetValue(ui_->XSpinBox, api.transform.get_x_cuts());
    QSpinBoxQuietSetValue(ui_->YSpinBox, api.transform.get_y_cuts());

    // XY accu visibility
    bool xy_visible = api.view.get_cuts_view_enabled();
    ...
    ui_->RenormalizeCheckBox->setChecked(api.global_pp.get_renorm_enabled());
    ui_->ReticleScaleDoubleSpinBox->setEnabled(api.contrast.get_reticle_display_enabled());
    ui_->ReticleScaleDoubleSpinBox->setValue(api.contrast.get_reticle_scale());
    ui_->DisplayReticleCheckBox->setChecked(api.contrast.get_reticle_display_enabled());
}
...
{
    API.transform.set_unwrapping_2d(value);
    parent_->notify();
}

void ViewPanel::update_3d_cuts_view(bool checked)
{
    if (API.view.set_3d_cuts_view(checked))
    {
...
{
    API.transform.set_fft_shift_enabled(value);
    parent_->notify();
}

void ViewPanel::set_registration(bool value)
{
    API.global_pp.set_registration_enabled(value);
    parent_->notify();
}

void ViewPanel::update_lens_view(bool checked)
{
    API.view.set_lens_view(checked);
    gui::set_lens_view(checked, parent_->auxiliary_window_max_size);
}

void ViewPanel::update_raw_view(bool checked)
{
    API.view.set_raw_view(checked);
    gui::set_raw_view(checked, parent_->auxiliary_window_max_size);
}

void ViewPanel::set_x_y()
{
    API.transform.set_x_y(ui_->XSpinBox->value(), ui_->YSpinBox->value());
    parent_->notify();
}

void ViewPanel::set_x_accu() { API.transform.set_x_accu_level(ui_->XAccSpinBox->value()); }

void ViewPanel::set_y_accu() { API.transform.set_y_accu_level(ui_->YAccSpinBox->value()); }

void ViewPanel::set_p(int value)
{
    API.transform.set_p_index(value);
    parent_->notify();
}

void ViewPanel::increment_p() { set_p(API.transform.get_p_index() + 1); }

void ViewPanel::decrement_p() { set_p(API.transform.get_p_index() - 1); }

void ViewPanel::set_p_accu()
{
    API.transform.set_p_accu_level(ui_->PAccSpinBox->value());
    parent_->notify();
}

void ViewPanel::set_q(int value)
{
    API.transform.set_q_index(value);
    parent_->notify();
}

void ViewPanel::set_q_acc()
{
    API.transform.set_q_accu_level(ui_->Q_AccSpinBox->value());
    parent_->notify();
}
...
void ViewPanel::set_log_enabled(const bool value) { API.contrast.set_log_enabled(value); }

void ViewPanel::set_accumulation_level(int value) { API.window_pp.set_accumulation_level(value); }

void ViewPanel::set_contrast_mode(bool value)
{
    API.contrast.set_contrast_enabled(value);
    parent_->notify();
}
...
void ViewPanel::set_contrast_invert(bool value) { API.contrast.set_contrast_invert(value); }

void ViewPanel::set_contrast_min(const double value) { API.contrast.set_contrast_min(value); }

void ViewPanel::set_contrast_max(const double value) { API.contrast.set_contrast_max(value); }

void ViewPanel::toggle_renormalize(bool value) { API.global_pp.set_renorm_enabled(value); }

void ViewPanel::display_reticle(bool value)
{
    API.contrast.set_reticle_display_enabled(value);
    gui::set_reticle_overlay_visible(value);
    parent_->notify();
}

void ViewPanel::reticle_scale(double value) { API.contrast.set_reticle_scale(value); }

void ViewPanel::update_registration_zone(double value)
{
    API.global_pp.update_registration_zone(value);
