...
{
    // XY view
    if (cd_.get_img_accu_slice_xy_enabled())
    {
        auto new_fd = fd_;
        ...
        allocate_accumulation_queue(image_acc_env_.gpu_accumulation_xy_queue,
                                    ...
                                    cd_.xy.img_accu_slice_level,
                                    new_fd);
    }

    // XZ view
    if (cd_.get_img_accu_slice_xz_enabled())
    {
        auto new_fd = fd_;
    ...
    // YZ view
    if (cd_.get_img_accu_slice_yz_enabled())
    {
        auto new_fd = fd_;
        new_fd.depth = sizeof(float);
...
void ImageAccumulation::insert_compute_average()
{
    auto compute_average_lambda = [&]() {
        // XY view
        if (image_acc_env_.gpu_accumulation_xy_queue && cd_.get_img_accu_slice_xy_enabled())
        ...
        // XZ view
        if (image_acc_env_.gpu_accumulation_xz_queue && cd_.get_img_accu_slice_xz_enabled())
        ...
        // YZ view
        if (image_acc_env_.gpu_accumulation_yz_queue && cd_.get_img_accu_slice_yz_enabled())
