...
{
    // XY view
    if (cd_.xy.img_acc_slice_enabled)
    {
        auto new_fd = fd_;
        ...
        allocate_accumulation_queue(image_acc_env_.gpu_accumulation_xy_queue,
                                    ...
                                    cd_.xy.img_acc_slice_level,
                                    new_fd);
    }

    // XZ view
    if (cd_.xz.img_acc_slice_enabled)
    {
        auto new_fd = fd_;
        ...
        allocate_accumulation_queue(image_acc_env_.gpu_accumulation_xz_queue,
                                    ...
                                    cd_.xz.img_acc_slice_level,
                                    new_fd);
    }

    // YZ view
    if (cd_.yz.img_acc_slice_enabled)
    {
        auto new_fd = fd_;
        new_fd.depth = sizeof(float);
...
void ImageAccumulation::insert_compute_average()
{
    auto compute_average_lambda = [&]() {
        // XY view
        if (image_acc_env_.gpu_accumulation_xy_queue && cd_.xy.img_acc_slice_enabled)
        ...
        // XZ view
        if (image_acc_env_.gpu_accumulation_xz_queue && cd_.xz.img_acc_slice_enabled)
        ...
        // YZ view
        if (image_acc_env_.gpu_accumulation_yz_queue && cd_.yz.img_acc_slice_enabled)
