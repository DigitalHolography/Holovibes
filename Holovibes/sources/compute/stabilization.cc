...
void Stabilization::compute_convolution(const float* x, const float* y, float* out)
{
    ...
    constexpr uint s = 64;
    convolution_float(x, y, out, zone.area(), plan2d_a, plan2d_b, plan2d_inverse);
}
    ...
    if (accumulation_queue_)
    {
        fn_vect_.push_back(
            [=]()
            {
                ...
                accumulate_images(static_cast<float*>(accumulation_queue_->get_buffer()),
            ...
            [=]()
            {
                ...
                if (cd_.img_acc_slice_xy_enabled)
                    cudaMemcpy(buffers_.gpu_float_buffer_,
                               float_buffer_average_,
                               accumulation_queue_->get_frame_desc().frame_size(),
