...
void BatchInputQueue::enqueue_multiple(const void* const frames, const int nb_frame, const cudaMemcpyKind memcpy_kind)
{
    ...
    if (curr_batch_counter_ + nb_frame > batch_size_)
    {
        ...
        enqueue_multiple(frames, slice, memcpy_kind);
        enqueue_multiple((void*)((char*)frames + (slice * fd_.get_frame_size())), nb_frame - slice, memcpy_kind);
        return;
    }
