...
void CircularVideoBuffer::add_new_frame(const float* const new_frame)
{
    ...
    if (nb_frames_ == buffer_capacity_)
    {
        ...
        subtract_frame_from_sum(sum_image_, oldest_frame, frame_res_, stream_);
        start_index_ = (start_index_ + 1) % buffer_capacity_;
    }

    // Add new frame to sum
    add_frame_to_sum(sum_image_, new_frame, frame_res_, stream_);
...
void CircularVideoBuffer::compute_mean_1_2(float* frame)
{
    cudaXMemsetAsync(compute_mean_1_2_buffer_, 0, sizeof(float) * nb_frames_, stream_);
    compute_multiplication_mean(compute_mean_1_2_buffer_.get(), data_.get(), frame, frame_res_, nb_frames_, stream_);
}
