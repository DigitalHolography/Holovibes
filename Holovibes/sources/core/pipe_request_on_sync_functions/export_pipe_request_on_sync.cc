...
void ExportPipeRequestOnSync::operator()<FrameRecordMode>(const FrameRecordStruct& new_value, Pipe& pipe)
{
    ...
    else if (new_value.record_mode == RecordMode::HOLOGRAM)
    {
    ...
    else if (new_value.record_mode == RecordMode::RAW)
    {
        pipe.get_frame_record_env().gpu_frame_record_queue_.reset(
    ...
    else if (new_value.record_mode == RecordMode::CUTS_XZ || new_value.record_mode == RecordMode::CUTS_YZ)
    {
        ...
        else if (new_value.record_mode == RecordMode::CUTS_YZ)
