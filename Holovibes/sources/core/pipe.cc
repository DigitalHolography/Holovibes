...
namespace holovibes
{
		...
		, fn_vect_()
	{
		...
		converts_ = std::make_unique<compute::Converts>(fn_vect_, buffers_, gpu_stft_buffer_, gpu_3d_vision, desc, input.get_frame_desc());
	...
	void Pipe::refresh()
	{
		...
		fn_vect_.push_back([=]() { queue_enqueue(buffers_.gpu_input_buffer_, gpu_stft_queue_); });
