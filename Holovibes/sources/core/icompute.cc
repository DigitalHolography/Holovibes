    ...
    , past_time_(std::chrono::high_resolution_clock::now())
{
    ...
    if (!buffers_.gpu_doppler_moment_zero.resize(buffers_.gpu_postprocess_frame_size))
        err++;

    if (!buffers_.gpu_doppler_moment_two.resize(buffers_.gpu_postprocess_frame_size))
        err++;
