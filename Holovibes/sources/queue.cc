    ...
    , has_overridden_(false)
{
    ...
    {
        LOG_ERROR(main, "Queue: could not allocate queue");
...
void Queue::resize(const unsigned int size, const cudaStream_t stream)
{
    ...
    if (max_size_ == 0 || !data_.resize(fd_.get_frame_size() * max_size_))
    {
        LOG_ERROR(main, "Queue: could not resize queue");
...
bool Queue::enqueue(void* elt, const cudaStream_t stream, cudaMemcpyKind cuda_kind)
{
    ...
    {
        LOG_ERROR(main, "Queue: could not enqueue: {}", std::string(cudaGetErrorString(cuda_status)));
        data_.reset();
        return false;
    }
...
void Queue::copy_multiple(Queue& dest, unsigned int nb_elts, const cudaStream_t stream)
{
    ...
    copy_multiple_aux(src, dst, static_cast<uint>(fd_.get_frame_size()), stream);
...
{
    cuda_tools::UniquePtr<uchar> src_uchar(fd_.get_frame_size());
