...
void Queue::copy_multiple_aux(QueueRegion& src, QueueRegion& dst, const size_t frame_size, const cudaStream_t stream, cudaMemcpyKind cuda_kind)
{
...
int Queue::dequeue(void* dest, const cudaStream_t stream, cudaMemcpyKind cuda_kind, int nb_elts)
{
    ...
    CHECK(nb_elts <= size_, "Request to dequeue {} elts, but the queue has only {}", nb_elts, size_);
