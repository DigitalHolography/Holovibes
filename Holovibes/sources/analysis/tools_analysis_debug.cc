...
void load_CSV_to_float_array_gpu(float* const output,
                                 ...
                                 cudaStream_t stream)
{
    float* data_csv_cpu = load_CSV_to_float_array(path);
    if (data_csv_cpu)
        cudaXMemcpyAsync(output, data_csv_cpu, size * sizeof(float), cudaMemcpyHostToDevice, stream);

    // Ensure the copy is done before deleting
