...
namespace holovibes
{
  ...
  void Pipe::request_refresh()
  {
    refresh_requested_ = true;
  }

  void Pipe::request_float_output(std::string& file_src, const unsigned int nb_frame)
  {
    try
    {
      ...
      request_refresh();
      std::cout << "[PIPE]: float record start." << std::endl;
    }
    catch (std::exception& e)
    {
      std::cout << "[PIPE]: float record: " << e.what() << std::endl;
      request_float_output_stop();
    }
  }

  void Pipe::request_float_output_stop()
  {
    if (float_output_file_.is_open())
      float_output_file_.close();
    float_output_requested_ = false;
    request_refresh();
    std::cout << "[PIPE]: float record done." << std::endl;
  }

  void Pipe::request_termination()
  {
  ...
  void Pipe::request_autocontrast()
  {
    autocontrast_requested_ = true;
    request_refresh();
  }

  void Pipe::request_stft_roi_update()
  {
    stft_update_roi_requested_ = true;
    request_update_n(compute_desc_.nsamples.load());
  }

  void Pipe::request_stft_roi_end()
  {
    stft_update_roi_requested_ = false;
    request_update_n(compute_desc_.nsamples.load());
  }

  void Pipe::request_autofocus()
  {
  ...
  void Pipe::request_autofocus_stop()
  {
    autofocus_stop_requested_ = true;
  }

  void Pipe::request_update_n(const unsigned short n)
  {
  ...
  void Pipe::request_average_record(
    ConcurrentDeque<std::tuple<float, float, float>>* output,
  ...
  void Pipe::average_record_caller(
    float* input,
    const unsigned int width,
    const unsigned int height,
    const Rectangle& signal,
  ...
  /* Looks like the pipe, but it searches for the right z value.
