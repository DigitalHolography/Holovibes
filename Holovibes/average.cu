...
static __global__ void kernel_zone_sum(	float		*input,
										const uint	width,
										float		*output,
										const uint	zone_start_x,
										const uint	zone_start_y,
															...
															cudaStream_t				stream)
{
	...
	uint signal_width = abs(signal.top_right.x - signal.top_left.x);
	uint signal_height = abs(signal.top_left.y - signal.bottom_left.y);
	uint noise_width = abs(noise.top_right.x - noise.top_left.x);
	uint noise_height = abs(noise.top_left.y - noise.bottom_left.y);
