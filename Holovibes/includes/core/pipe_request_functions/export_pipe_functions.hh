...
class ExportPipeRequest
{
  public:
    ...
    template <>
    void operator()<FrameRecord>(const FrameRecord& new_value, const FrameRecord& old_value, Pipe& pipe)
    {
        ...
        else if (new_value.get_record_mode() == RecordMode::CUTS_XZ ||
                 new_value.get_record_mode() == RecordMode::CUTS_YZ)
        {
    ...
    template <>
    void operator()<ChartRecord>(const ChartRecord& new_value, const ChartRecord& old_value, Pipe& pipe)
    {
