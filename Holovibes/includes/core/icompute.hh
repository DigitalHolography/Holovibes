...
struct CoreBuffersEnv
{
    ...
    cuda_tools::UniquePtr<cufftComplex> gpu_spatial_transformation_buffer =
        nullptr;

    /** Float buffer. Contains only one frame. We fill it with the correct
     * computed p frame converted to float. */
    cuda_tools::UniquePtr<float> gpu_postprocess_frame = nullptr;
    /** Size in components (size in byte / sizeof(float)) of the
     gpu_postprocess_frame. Could be removed by changing gpu_postprocess_frame
     type to cuda_tools::Array. */
    unsigned int gpu_postprocess_frame_size = 0;
    /** Float XZ buffer. Contains only one frame. We fill it with the correct
     * computed p XZ frame. */
    cuda_tools::UniquePtr<float> gpu_postprocess_frame_xz = nullptr;
    /** Float YZ buffer. Contains only one frame. We fill it with the correct
     * computed p YZ frame. */
    cuda_tools::UniquePtr<float> gpu_postprocess_frame_yz = nullptr;

    /** Unsigned Short output buffer. Contains only one frame, inserted after
     * all postprocessing on float_buffer */
    cuda_tools::UniquePtr<unsigned short> gpu_output_frame = nullptr;
    /** Unsigned Short XZ output buffer. Contains only one frame, inserted after
     * all postprocessing on float_buffer_cut_xz */
    cuda_tools::UniquePtr<unsigned short> gpu_output_frame_xz = nullptr;
    /** Unsigned Short YZ output buffer. Contains only one frame, inserted after
     * all postprocessing on float_buffer_cut_yz */
...
struct TimeTransformationEnv
{
    /** STFT Queue. Constains time_transformation_size frames. It accumulates
     input frames after spatial fft,
     in order to apply STFT only when the frame counter is equal to
     time_transformation_stride. */
    std::unique_ptr<Queue> gpu_time_transformation_queue = nullptr;
    /** STFT buffer. Contains time_transformation_size frames. Contains the
     * result of the STFT done on the STFT queue. */
    cuda_tools::UniquePtr<cufftComplex> gpu_p_acc_buffer = nullptr;
    /** STFT XZ Queue. Contains the ouput of the STFT on slice XZ. Enqueued with
     * gpu_float_buffer or gpu_ushort_buffer. */
    std::unique_ptr<Queue> gpu_output_queue_xz = nullptr;
    /** STFT YZ Queue. Contains the ouput of the STFT on slice YZ. Enqueued with
     * gpu_float_buffer or gpu_ushort_buffer. */
...
class ICompute : public Observable
{
  ...
  protected:
    ...
    /** Counting pipe iteration, in order to update fps only every 100
     * iterations. */
