...
class Pipe : public ICompute
{
  public:
    ...
    */
    ...
    /*! \brief Calls autocontrast on the *next* pipe iteration on the wanted
       view
  ...
  protected:
     ...
     * If Holovibes crash in a cuda function right after updating something on
     * the GUI, It probably means that the ComputeDescriptor has been updated
     * before the end of the iteration. The pipe uses the ComputeDescriptor, and
     * is refresh only at the end of the iteration. You
     * **must** wait until the end of the refresh, or use the insert_fn_end_vect
     * function to update the ComputeDescriptor, otherwise the end of the
     * current iteration will be wrong, and will maybe crash. */
    virtual void exec();
    ...
    */
    ...
    /*!
    ** \brief Transfer from gpu_space_transformation_buffer to
    *gpu_time_transformation_queue for time transformation
    */
    ...
    */
    ...
    /*!
    ** \brief Reset the batch index if time_transformation_stride has been
    *reached
    */
  ...
  private:
    ...
    //! Vecor of functions that will be executed once, after the execution of
    //! fn_compute_vect_.
    FunctionVector fn_end_vect_;
    /*! Mutex that prevents the insertion of a function during its execution.
        Since we can insert functions in fn_end_vect_ from other threads
       (MainWindow), we need to lock it.
    */
