...
/*! \brief Returns the rotation of the specified window. The rotation is in degrees and is applied anti-clockwise. Only
...
/*! \brief Returns the rotation of the current window. The rotation is in degrees and is applied anti-clockwise. Only 0,
 * 90, 180 and 270 degrees are supported.
 *
 * \return float the rotation of the window
 */
inline float get_rotation() { return get_rotation(get_current_window_type()); }

/*! \brief Sets the rotation of the specified window. The rotation is in degrees and is applied anti-clockwise. Only 0,
 * 90, 180 and 270 degrees are supported.
 *
 * \param[in] kind the kind of window
...
/*! \brief Sets the rotation of the current window. The rotation is in degrees and is applied anti-clockwise. Only 0,
 * 90, 180 and 270 degrees are supported.
 *
 * \param[in] value the new rotation of the window either: 0, 90, 180 or 270 degrees.
 */
...
/*! \brief Returns the size of the accumulation window for the specified window. The accumulation is a post processing
 * step that takes `get_accumulation_level` images as input and average them to output a single image.
 *
 * High values of accumulation level will reduce the noise in the image but will also reduce the frame rate and increase
 * motion artifacts.
 * An accumulation level of 1 will disable the accumulation.
 *
 * \param[in] kind the kind of window
 *
 ...
 *
 * The accumulation is a post processing step that takes `get_accumulation_level` images as input and average them to
 * output a single image. High values of accumulation level will reduce the noise in the image but will also reduce the
 * frame rate and increase motion artifacts. An accumulation level of 1 will disable the accumulation.
 *
 * \param[in] kind the kind of window
...
/*! \brief Sets value of raw bit shift. When raw data is displayed, pixels will be equal to: `px = px * 2^RawBitshift`
 *
