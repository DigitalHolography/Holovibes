	...
	public:
		...
		void go(size_t bufferCount) {
			...
			//****************************************************************************************************************
				...
				grabbers[0]->setString<DeviceModule>("CycleMinimumPeriod", "20000.0");  // set the trigger rate to 250K Hz
...
int main() {
	...
	Exptime = &Expvalue;
	*Exptime = 9000;
	SubGrabberManager subGrabberManager(genTL, BUFFER_COUNT, 1024, 512, "Mono8");
	step = 1;
	subGrabberManager.grabbers[0]->setFloat<RemoteModule>("ExposureTime", 9000);
